...
# Find const MVPTarget and const ResultsPlaceholder
pattern = re.compile(r"const MVPTarget = \(\) => \(.*?\);\s*const ResultsPlaceholder", re.DOTALL | re.MULTILINE)
if pattern.search(content):
    content = pattern.sub(new_mvp_target + "\n        const ResultsPlaceholder", content)
else:
    print("Failed to find MVPTarget component")

# Also replace <MVPTarget /> with <InteractiveMVPTarget /> in App
content = content.replace("<MVPTarget />", "<InteractiveMVPTarget />")
